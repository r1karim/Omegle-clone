...
export default function () {
  ...
  const listMessages = messages.map((message) => (
    <div>
      {message.sender ? (
        <span style={{ display: "inline", color: "skyblue" }}>You: </span>
      ) : (
        <span style={{ display: "inline", color: "red" }}>Anonymous: </span>
      )}
      {message.content}
    </div>
  ));
        ...
        <div className={style.textChat}>
          <div className={style.textBox}>
            <span>
              {" "}
              {anon
                ? "You're in a chat with a random stranger."
                : "Looking for a match..."}
            </span>
            <div className={style.messages}>{listMessages}</div>
          </div>
